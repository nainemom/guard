...
import Form, { type FormErrors } from '@/components/form/Form';
...
export default function PersonalKeysEditForm({
  onSuccess,
  onClose,
}: PersonalKeysEditFormProps) {
  const [personalKeys, setPersonalKeys] = useAuth();

  const [formData, setFormData] = useState<Partial<CryptographyPairKeys>>(
    personalKeys || {},
  );
  useEffect(() => {
  ...
  const formValidator = useCallback(
    (submittedFormData: Partial<CryptographyPairKeys>) => {
  ...
  const handleFormInput = useCallback(
    (newFormData: Partial<CryptographyPairKeys>) => {
  ...
  const handleFormSubmit = useCallback(
    ...
    [onSuccess, requestClose, setPersonalKeys],
  );

  return (
    <>
      <DialogTitle
        title="Edit Personal Keys"
        closeButton
      ...
      >
        <DialogBody className="p-3">
          ...
          <div>
            <Input
              className="text-danger-normal h-52"
              name="private_key"
              size="manual"
              multiLine
              ...
              label="Private Key:"
            />
            <p className="mt-3 text-xs text-body-subtitle">
              Never share this value to anyone!
            </p>
          </div>
        </DialogBody>
        <DialogButtons>
