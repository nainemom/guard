...
import type { CryptographyPairKeys } from './cryptography';

export const createAuthFile = (pairKeys: CryptographyPairKeys) =>
  new File(
...
export const openAuthFile = async (file: File): Promise<CryptographyPairKeys> =>
  JSON.parse(ab2str(await file.arrayBuffer()));
...
export const openEncryptedFile = async (
  file: File,
): Promise<[ArrayBuffer, string]> => {
  const ab = await file.arrayBuffer();
  const name = ((file as File)?.name || 'unknown.file').replace(
    new RegExp(`.${ENCRYPTED_FILE_EXT}$`, 'g'),
    '',
  );
  return [ab, name];
};

export const exportFileToUser = (file: File): Promise<void> =>
  new Promise((resolve) => {
