...
export const ubtoa = (buffer: ArrayBuffer) =>
  btoa(new Uint8Array(buffer).reduce((p, b) => p + String.fromCharCode(b), ''));

// https://stackoverflow.com/a/43271130
export const uatob = (str: string) =>
...
export const abs2ab = (abs: ArrayBuffer[]): ArrayBuffer => {
  const ret = new ArrayBuffer(
    abs.reduce((prev, current) => prev + current.byteLength, 0),
  );
